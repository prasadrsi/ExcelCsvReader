...
namespace Excel
{
    public class ExcelOpenXmlReader : IExcelDataReader
    {
                    ...
                    if (_xmlReader.NodeType == XmlNodeType.Text && hasValue)
                    {
                        ...
                        else if (null != a_s)
                        {
                            ...
                            if (xf.ApplyNumberFormat && IsDateTimeStyle(xf.NumFmtId) && o != null && o.ToString() != string.Empty)
                            {
                                try
                                {
                                    o = DateTime.FromOADate(Convert.ToDouble(o, CultureInfo.InvariantCulture)).ToShortDateString();
                                }
                                catch
                                {
                ...
                else if (ReadSheetRow(_workbook.Sheets[_resultIndex]))
                {
                    for (int index = 0; index < _cellsValues.Length; index++)
                    {
                        if (_cellsValues[index] != null && _cellsValues[index].ToString().Trim().Length > 0)
                            try
                            {
                                table.Columns.Add(_cellsValues[index].ToString().Trim());
                            }
        ...
        public string[] GetHeaders()
        {
            ...
            if (_isFirstRowAsColumnNames && ReadSheetRow(_workbook.Sheets[_resultIndex]) || (_isFirstRowAsColumnNames && RecordCount == 1))
            {
                for (int i = 0; i < FieldCount; i++)
                {
                    if (_cellsValues[i] != null && _cellsValues[i].ToString().Length > 0)

                        headers[i] = _cellsValues[i].ToString();
                }
            }
            return headers;
        }
        ...
        {
            get { return (_resultIndex >= 0 && _resultIndex < ResultsCount) ? _workbook.Sheets[_resultIndex].RowsCount : -1; }
        }
    }
}
