    ...
    public class ExcelBinaryReader : IExcelDataReader
    {
            ...
            while (null != (rec = m_stream.Read()))
            {
                switch (rec.ID)
                {
                    ...
                    case BIFFRECORDTYPE.XF_V2:
                        m_globals.ExtendedFormats.Add((XlsBiffXF)rec);
                        break;
        ...
        private void pushCellValue(XlsBiffBlankCell cell)
        {
            ...
            switch (cell.ID)
            {
                ...
                _dValue : tryConvertOADateTime(_dValue, cell.GetFormat(m_globals.ExtendedFormats));
                    break;
                ...
                case BIFFRECORDTYPE.RK:
                    XlsBiffRKCell _rkCell = (XlsBiffRKCell)cell;
                    _dValue = (_rkCell).Value;
                    //_dValue = ((XlsBiffRKCell)cell).Value;

                    m_cellsValues[cell.ColumnIndex] = !m_ConvertOADate ?
                        //_dValue : tryConvertOADateTime(_dValue, cell.XFormat);
                         _dValue : tryConvertOADateTime(_dValue, _rkCell.GetFormat(m_globals.ExtendedFormats));
                    break;
                case BIFFRECORDTYPE.MULRK:
                    ...
                    //}
                    XlsBiffMulRKCell _mulRkCell = (XlsBiffMulRKCell)cell;
                    
                    for (ushort j = cell.ColumnIndex; j <= _mulRkCell.LastColumnIndex; j++)
                    {
                        //					m_cellsValues[j] = _rkCell.GetValue(j);
                        _dValue = _mulRkCell.GetValue(j);

                        m_cellsValues[j] = !m_ConvertOADate ?
                           _dValue : tryConvertOADateTime(_dValue, m_globals.ExtendedFormats[_mulRkCell.GetXF(j)].Format);
                    }
                    break;
                ...
                case BIFFRECORDTYPE.FORMULA_BIFF3:
                case BIFFRECORDTYPE.FORMULA_OLD:
                    ...
                    {
                        _oValue = null;
                        m_cellsValues[cell.ColumnIndex] = "#REF!";
                    }
                    else
                    {
                        m_cellsValues[cell.ColumnIndex] = !m_ConvertOADate ?
                            //_oValue : tryConvertOADateTime(_oValue, (ushort)(cell.XFormat + 75));//date time offset
                             _oValue : tryConvertOADateTime(_oValue, cell.GetFormat(m_globals.ExtendedFormats));
                    }
       ...
       private static object tryConvertOADateTime(double value, ushort cellFormat)
        {
            switch (cellFormat)
            {
                //Time format
                case 18:
               case 20:
                    return DateTime.FromOADate(value).ToShortTimeString();
               case 19:
                case 21:
                   return DateTime.FromOADate(value).ToLongTimeString();
                //TimeDate Format
                case 22:
                    return DateTime.FromOADate(value).ToString(System.Globalization.CultureInfo.CurrentCulture);
                //DateFormat
                case 14:
                case 15:
                case 16:
...
//-						: time.ToLongTimeString();
        ...
        {
            string[] headers = new string[m_maxCol];
            ...
            if (_isFirstRowAsColumnNames && readWorkSheetRow() || (_isFirstRowAsColumnNames && m_maxRow == 1))
            {
                for (int i = 0; i < m_maxCol; i++)
                {
                    if (m_cellsValues[i] != null && m_cellsValues[i].ToString().Length > 0)
                        ...
                        headers[i] = m_cellsValues[i].ToString();
                }
            }
            return headers;
        }
